...
public class ItemInfo
{
    ...
    // Good/Evil System - Item Requirements
    public uint RequiredGoodLevel = 0;
    public uint RequiredEvilLevel = 0;

    // Attribute System - Item Requirements
    // Dictionary mapping Attribute to required level/points
    public Dictionary<Attribute, uint> RequiredAttributes = new Dictionary<Attribute, uint>();
    ...
    public ItemInfo(BinaryReader reader, int version = int.MaxValue, int customVersion = int.MaxValue)
    {
        ...
        // Good/Evil System - Load item requirements
        if (version > 110)
        {
            RequiredGoodLevel = reader.ReadUInt32();
            RequiredEvilLevel = reader.ReadUInt32();
        }

        // Attribute System - Load attribute requirements
        if (version > 111)
        {
            int attributeCount = reader.ReadInt32();
            RequiredAttributes = new Dictionary<Attribute, uint>();
            for (int i = 0; i < attributeCount; i++)
            {
                Attribute attr = (Attribute)reader.ReadByte();
                uint requiredValue = reader.ReadUInt32();
                RequiredAttributes[attr] = requiredValue;
            }
        }
    }
    ...
    public void Save(BinaryWriter writer)
    {
        ...
        // Good/Evil System - Save item requirements
        writer.Write(RequiredGoodLevel);
        writer.Write(RequiredEvilLevel);

        // Attribute System - Save attribute requirements
        writer.Write(RequiredAttributes != null ? RequiredAttributes.Count : 0);
        if (RequiredAttributes != null)
        {
            foreach (var attrReq in RequiredAttributes)
            {
                writer.Write((byte)attrReq.Key);
                writer.Write(attrReq.Value);
            }
        }
