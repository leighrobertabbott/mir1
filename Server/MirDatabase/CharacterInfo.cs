...
namespace Server.MirDatabase
{
    public class CharacterInfo
    {
        ...
        // Good/Evil System - Independent level system
        public uint GoodLevel = 0;
        public ulong GoodExperience = 0;
        public uint EvilLevel = 0;
        public ulong EvilExperience = 0;

        public UserItem[] Inventory = new UserItem[36], Equipment = new UserItem[8], Trade = new UserItem[10], QuestInventory = new UserItem[30], Refine = new UserItem[16];
        ...
        public CharacterInfo(ClientPackets.NewCharacter p, MirConnection c)
        {
            ...
            Class = p.Class;
            Gender = p.Gender;
            Hair = Math.Clamp(p.Hair, (byte)1, (byte)21);

            CreationIP = c.IPAddress;
            CreationDate = Envir.Now;

            // Attribute System - Initialize all attributes for new characters
        ...
        public virtual void Load(BinaryReader reader, int version, int customVersion)
        {
            ...
            if (version > 109) // Good/Evil System
            {
                MentalStateLvl = reader.ReadByte();
                GoodLevel = reader.ReadUInt32();
                GoodExperience = reader.ReadUInt64();
                EvilLevel = reader.ReadUInt32();
                EvilExperience = reader.ReadUInt64();
            }
        ...
        public virtual void Save(BinaryWriter writer)
        {
            ...
            writer.Write(MentalStateLvl);
            // Good/Evil System - Save independent levels
            writer.Write(GoodLevel);
            writer.Write(GoodExperience);
            writer.Write(EvilLevel);
            writer.Write(EvilExperience);
        ...
        public SelectInfo ToSelectInfo()
        {
            return new SelectInfo
            {
                ...
                Gender = Gender,
                Hair = Hair,
                Body = Equipment[(int)EquipmentSlot.Armour]?.Info?.Shape ?? 0,
