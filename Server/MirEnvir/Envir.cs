...
namespace Server.MirEnvir
{
    ...
    public class Envir
    {
        ...
        public void NewCharacter(ClientPackets.NewCharacter p, MirConnection c, bool IsGm)
        {
            ...
            // Attribute System - Class is now optional (defaults to Warrior for backward compatibility)
            // Class is determined by attribute distribution, but we keep it for display/legacy purposes
            if (p.Class != MirClass.Warrior && p.Class != MirClass.Wizard && p.Class != MirClass.Taoist)
            {
                // Default to Warrior if invalid class provided (for backward compatibility)
                p.Class = MirClass.Warrior;
            }
