...
namespace Server.MirObjects
{
    public class PlayerObject : HumanObject
    {
        ...
        public override void GainExp(uint amount)
        {
            ...
            // Good/Evil System - Experience distribution handled in WinExp method
            ...
            // Good/Evil System - Check and level up Good/Evil
            CheckGoodEvilLevelUp();
        ...
        {
            // Good/Evil System - Check map requirements
            if (temp != null && temp.Info != null)
            {
                if (temp.Info.RequiredGoodLevel > 0 && Info.GoodLevel < temp.Info.RequiredGoodLevel)
                {
                    ReceiveChat($"You need Good level {temp.Info.RequiredGoodLevel} to enter this map.", ChatType.System);
                    return false;
                }

                if (temp.Info.RequiredEvilLevel > 0 && Info.EvilLevel < temp.Info.RequiredEvilLevel)
                {
                    ReceiveChat($"You need Evil level {temp.Info.RequiredEvilLevel} to enter this map.", ChatType.System);
                    return false;
                }
            }
        ...
        public void InspectPlayer(uint objectID)
        {
            ...
            for (int i = 0; i < CurrentMap.Players.Count; i++)
            {
                if (CurrentMap.Players[i].ObjectID == objectID)
                {
                    targetPlayer = CurrentMap.Players[i];
                    break;
                }
            }
            ...
            {
                ReceiveChat("Player not found or cannot inspect yourself.", ChatType.System);
                return;
            }

            // Check range
            if (!Functions.InRange(CurrentLocation, targetPlayer.CurrentLocation, Globals.DataRange))
            {
                ReceiveChat("Player is too far away.", ChatType.System);
                return;
            }

            // Send inspect data
            S.InspectPlayerResponse response = new S.InspectPlayerResponse
            {
                ObjectID = targetPlayer.ObjectID,
                Name = targetPlayer.Name,
                Level = targetPlayer.Level,
                Class = targetPlayer.Class,
                Gender = targetPlayer.Gender,
                Hair = targetPlayer.Hair,
                Equipment = targetPlayer.Info.Equipment,
        ...
        #region Good/Evil System
        /// <summary>
        /// Distributes Good/Evil experience based on monster alignment
        /// </summary>
        public void DistributeGoodEvilExperience(uint experience, MonsterInfo monsterInfo)
        {
            if (monsterInfo == null) return;

            // Neutral monsters give both Good and Evil experience
            if (monsterInfo.IsNeutral)
            {
                GainGoodExperience(experience / 2);
                GainEvilExperience(experience / 2);
            }
            else if (monsterInfo.IsGood)
            {
                // Good monsters give Good experience
                GainGoodExperience(experience);
            }
            else
            {
                // Evil monsters give Evil experience
                GainEvilExperience(experience);
            }
        }

        /// <summary>
        /// Gain Good experience and check for level up
        /// </summary>
        public void GainGoodExperience(ulong amount)
        {
            if (amount == 0) return;

            Info.GoodExperience += amount;
            CheckGoodLevelUp();
        }

        /// <summary>
        /// Gain Evil experience and check for level up
        /// </summary>
        public void GainEvilExperience(ulong amount)
        {
            if (amount == 0) return;

            Info.EvilExperience += amount;
            CheckEvilLevelUp();
        }

        /// <summary>
        /// Check if Good level should increase
        /// </summary>
        private void CheckGoodLevelUp()
        {
            ulong maxExp = GetMaxGoodEvilExperience(Info.GoodLevel);
            
            while (Info.GoodExperience >= maxExp && Info.GoodLevel < uint.MaxValue)
            {
                Info.GoodExperience -= maxExp;
                Info.GoodLevel++;
                maxExp = GetMaxGoodEvilExperience(Info.GoodLevel);
                
                ReceiveChat($"Your Good alignment has increased to level {Info.GoodLevel}!", ChatType.System);
                RefreshStats(); // Good/Evil level may affect stats
            }
        }

        /// <summary>
        /// Check if Evil level should increase
        /// </summary>
        private void CheckEvilLevelUp()
        {
            ulong maxExp = GetMaxGoodEvilExperience(Info.EvilLevel);
            
            while (Info.EvilExperience >= maxExp && Info.EvilLevel < uint.MaxValue)
            {
                Info.EvilExperience -= maxExp;
                Info.EvilLevel++;
                maxExp = GetMaxGoodEvilExperience(Info.EvilLevel);
                
                ReceiveChat($"Your Evil alignment has increased to level {Info.EvilLevel}!", ChatType.System);
                RefreshStats(); // Good/Evil level may affect stats
            }
        }

        /// <summary>
        /// Check both Good and Evil for level up (called after character level up)
        /// </summary>
        private void CheckGoodEvilLevelUp()
        {
            CheckGoodLevelUp();
            CheckEvilLevelUp();
        }
        ...
        /// <summary>
        /// Calculate maximum experience needed for Good/Evil level
        /// Formula similar to character level but can be adjusted
        /// </summary>
        private ulong GetMaxGoodEvilExperience(uint level)
        {
            // Base formula: 1000 * level^2 (can be adjusted in Settings)
            return (ulong)(1000 * level * level);
        }
        #endregion
    }
}