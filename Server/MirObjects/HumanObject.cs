...
namespace Server.MirObjects
{
    public class HumanObject : MapObject
    {
        ...
        protected bool CanUseItem(UserItem item)
        {
            ...
            // Attribute System - Check attribute requirements first (new system)
            if (item.Info.RequiredAttributes != null && item.Info.RequiredAttributes.Count > 0)
            {
                foreach (var attrReq in item.Info.RequiredAttributes)
                {
                    if (!AttributeValues.ContainsKey(attrReq.Key))
                    {
                        ReceiveChat($"You need {attrReq.Key} attribute level {attrReq.Value} to use this item.", ChatType.System);
                        return false;
                    }
                    
                    UserAttribute attr = AttributeValues[attrReq.Key];
                    // Check both level and points (total investment)
                    uint totalValue = attr.Level + attr.Points;
                    if (totalValue < attrReq.Value)
                    {
                        ReceiveChat($"You need {attrReq.Key} attribute level {attrReq.Value} to use this item. (You have {totalValue})", ChatType.System);
                        return false;
                    }
                }
            }
            // Legacy class requirement check (backward compatibility)
            else if (item.Info.RequiredClass != RequiredClass.None)
            {
                // Convert class requirement to attribute-based check
                MirClass requiredClassRole = Shared.Data.AttributeStatCalculator.DetermineClassRole(AttributeValues);
                ...
                switch (Class)
                {
                    case MirClass.Warrior:
                        ...
                        {
                            // Check if attributes match warrior-like build
                            if (requiredClassRole != MirClass.Warrior)
                            {
                                ReceiveChat("This item requires a warrior-like attribute build.", ChatType.System);
                                return false;
                            }
                        }
                        break;
                    case MirClass.Wizard:
                        ...
                        {
                            if (requiredClassRole != MirClass.Wizard)
                            {
                                ReceiveChat("This item requires a wizard-like attribute build.", ChatType.System);
                                return false;
                            }
                        }
                        break;
                    case MirClass.Taoist:
                        ...
                        {
                            if (requiredClassRole != MirClass.Taoist)
                            {
                                ReceiveChat("This item requires a taoist-like attribute build.", ChatType.System);
                                return false;
                            }
                        }
                        break;
            ...
            // Good/Evil System - Check Good/Evil level requirements
            if (item.Info.RequiredGoodLevel > 0 && Info.GoodLevel < item.Info.RequiredGoodLevel)
            {
                ReceiveChat($"You need Good level {item.Info.RequiredGoodLevel} to use this item.", ChatType.System);
                return false;
            }

            if (item.Info.RequiredEvilLevel > 0 && Info.EvilLevel < item.Info.RequiredEvilLevel)
            {
                ReceiveChat($"You need Evil level {item.Info.RequiredEvilLevel} to use this item.", ChatType.System);
                return false;
            }
        ...
        {
            // Attribute System - Check attribute requirements first (new system)
            if (info.RequiredAttributes != null && info.RequiredAttributes.Count > 0)
            {
                foreach (var attrReq in info.RequiredAttributes)
                {
                    if (!AttributeValues.ContainsKey(attrReq.Key))
                        return false;
            ...
            // Legacy class requirement check (backward compatibility)
            else if (info.RequiredClass != RequiredClass.None)
            {
                MirClass requiredClassRole = Shared.Data.AttributeStatCalculator.DetermineClassRole(AttributeValues);
                ...
                {
                    case MirClass.Warrior:
                        if (!info.RequiredClass.HasFlag(RequiredClass.Warrior))
                        {
                            if (requiredClassRole != MirClass.Warrior) return false;
                        }
                        break;
                    case MirClass.Wizard:
                        if (!info.RequiredClass.HasFlag(RequiredClass.Wizard))
                        {
                            if (requiredClassRole != MirClass.Wizard) return false;
                        }
                        break;
                    case MirClass.Taoist:
                        if (!info.RequiredClass.HasFlag(RequiredClass.Taoist))
                        {
                            if (requiredClassRole != MirClass.Taoist) return false;
                        }
                        break;
                    default:
                        return false;
        ...
        {
            // Attribute System - Use attribute-determined class role for BaseStats (for backward compatibility)
            // The actual stats are calculated from attributes in RefreshLevelStats
            MirClass classRole = Shared.Data.AttributeStatCalculator.DetermineClassRole(AttributeValues);
            Enqueue(new S.BaseStatsInfo { Stats = Settings.ClassBaseStats[(byte)classRole] });
        }
        ...
        protected void RefreshLevelStats()
        {
            ...
            // Attribute-based stat calculation (replaces class-based system)
            if (AttributeValues != null && AttributeValues.Count > 0)
            {
                Stats calculatedStats = Shared.Data.AttributeStatCalculator.CalculateAllStats(AttributeValues, Level);
                
                // Apply calculated stats
                foreach (var statPair in calculatedStats.Values)
                {
                    Stats[statPair.Key] = statPair.Value;
                }
            }
            else
            {
                // Fallback to class-based for backward compatibility (legacy characters)
        ...
        private void RefreshEquipmentStats()
        {
            ...
            for (int i = 0; i < Info.Equipment.Length; i++)
            {
                ...
                // Attribute System - Use attribute-determined class role for item variants
                MirClass itemClassRole = Shared.Data.AttributeStatCalculator.DetermineClassRole(AttributeValues);
                ItemInfo realItem = Functions.GetRealItem(temp.Info, Info.Level, itemClassRole, Envir.ItemInfoList);
        ...
        {
            // Attribute System - Use attribute-determined class role for stat caps
            MirClass classRole = Shared.Data.AttributeStatCalculator.DetermineClassRole(AttributeValues);
            foreach (var cap in Settings.ClassBaseStats[(byte)classRole].Caps.Values)
            {
        ...
        public void RangeAttack(MirDirection dir, Point location, uint targetID)
        {
            ...
            // Attribute System - Use attribute-determined class role for item variants
            MirClass itemClassRole = Shared.Data.AttributeStatCalculator.DetermineClassRole(AttributeValues);
            ItemInfo RealItem = Functions.GetRealItem(Info.Equipment[(int)EquipmentSlot.Weapon].Info, Info.Level, itemClassRole, Envir.ItemInfoList);
