...
namespace Client.MirScenes
{
    public sealed class GameScene : MirScene
    {
        ...
        public GameScene()
        {
            ...
            InspectDialog = new InspectDialog { Parent = this };
        ...
        public override void ProcessPacket(Packet p)
        {
            switch (p.Index)
            {
                ...
                case (short)ServerPacketIds.ChatItemStats:
                    ...
                    break;
                case (short)ServerPacketIds.InspectPlayerResponse:
                    InspectPlayerResponse((S.InspectPlayerResponse)p);
                    break;
        ...
        private void InspectPlayerResponse(S.InspectPlayerResponse p)
        {
            // Create or show inspect dialog
            if (InspectDialog == null)
            {
                InspectDialog = new InspectDialog();
                InspectDialog.Parent = this;
            }

            InspectDialog.ShowInspect(p);
        }
        ...
        public MirControl NameInfoLabel(UserItem item, bool inspect = false, bool hideDura = false)
        {
            ...
            MirClass job = Shared.Data.AttributeStatCalculator.DetermineClassRole(MapObject.User.AttributeValues);
            HoverItem = item;
        ...
        public MirControl NeedInfoLabel(UserItem item, bool Inspect = false)
        {
            ...
            #region ATTRIBUTE/CLASS REQUIREMENTS
            // Attribute System - Display attribute requirements
            if (realItem.RequiredAttributes != null && realItem.RequiredAttributes.Count > 0)
            {
                foreach (var attrReq in realItem.RequiredAttributes)
                {
                    count++;
                    Color colour = Color.White;
                    
                    if (MapObject.User.AttributeValues != null && MapObject.User.AttributeValues.ContainsKey(attrReq.Key))
                    {
                        UserAttribute attr = MapObject.User.AttributeValues[attrReq.Key];
                    ...
                    MirLabel ATTRLabel = new MirLabel
                    {
                        AutoSize = true,
                        ForeColour = colour,
                        Location = new Point(4, ItemLabel.DisplayRectangle.Bottom),
                        OutLine = true,
                        Parent = ItemLabel,
                        Text = $"Requires {attrReq.Key}: {attrReq.Value}"
                    };

                    ItemLabel.Size = new Size(Math.Max(ItemLabel.Size.Width, ATTRLabel.DisplayRectangle.Right + 4),
                        Math.Max(ItemLabel.Size.Height, ATTRLabel.DisplayRectangle.Bottom));
                }
            }
            // Legacy class requirement display (backward compatibility)
            else if (realItem.RequiredClass != RequiredClass.None)
            {
                count++;
                Color colour = Color.White;

                MirClass attributeRole = Shared.Data.AttributeStatCalculator.DetermineClassRole(MapObject.User.AttributeValues);
                ...
                switch (MapObject.User.Class)
                {
                    case MirClass.Warrior:
                        ...
                        {
                            if (attributeRole != MirClass.Warrior)
                                colour = Color.Red;
                        }
                        break;
                    case MirClass.Wizard:
                        ...
                        {
                            if (attributeRole != MirClass.Wizard)
                                colour = Color.Red;
                        }
                        break;
                    case MirClass.Taoist:
                        ...
                        {
                            if (attributeRole != MirClass.Taoist)
                                colour = Color.Red;
                        }
                        break;
                }
            ...
            if (MouseControl == this)
            {
                ...
                switch (MapButtons)
                {
                    case MouseButtons.Left:
                        ...
                        break;
                    case MouseButtons.Right:
                        // Right-click on player to inspect (Ctrl+Right for other actions)
                        if (MapObject.MouseObject is PlayerObject playerTarget && playerTarget != User)
                        {
                            if (CMain.Ctrl)
                            {
                                // Ctrl+Right click - allow other actions
                                break;
                            }
                            else
                            {
                                // Regular right-click - inspect player
                                Network.Enqueue(new C.InspectPlayer { ObjectID = playerTarget.ObjectID });
                                return;
                            }
                        }
