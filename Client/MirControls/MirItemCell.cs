    ...
    public sealed class MirItemCell : MirImageControl
    {
            ...
            get
            {
                switch (GridType)
                {
                    ...
                    case MirGridType.Inspect:
                        return InspectDialog.InspectEquipment;
        ...
        private bool CanUseItem()
        {
            ...
            if (Item.Info.RequiredAttributes != null && Item.Info.RequiredAttributes.Count > 0)
            {
                ...
                {
                    if (actor.AttributeValues == null || !actor.AttributeValues.ContainsKey(attrReq.Key))
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat($"You need {attrReq.Key} attribute level {attrReq.Value} to use this item.", ChatType.System);
                        return false;
                    }
                    
                    UserAttribute attr = actor.AttributeValues[attrReq.Key];
                    uint totalValue = attr.Level + attr.Points;
                    if (totalValue < attrReq.Value)
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat($"You need {attrReq.Key} attribute level {attrReq.Value} to use this item. (You have {totalValue})", ChatType.System);
                        return false;
                    }
                }
            }
            ...
            {
                MirClass requiredClassRole = Shared.Data.AttributeStatCalculator.DetermineClassRole(actor.AttributeValues);
                ...
                switch (actor.Class)
                {
                    case MirClass.Warrior:
                        if (!Item.Info.RequiredClass.HasFlag(RequiredClass.Warrior))
                        {
                            ...
                            {
                                GameScene.Scene.ChatDialog.ReceiveChat("This item requires a warrior-like attribute build.", ChatType.System);
                                return false;
                            }
                        }
                        break;
                    case MirClass.Wizard:
                        if (!Item.Info.RequiredClass.HasFlag(RequiredClass.Wizard))
                        {
                            ...
                            {
                                GameScene.Scene.ChatDialog.ReceiveChat("This item requires a wizard-like attribute build.", ChatType.System);
                                return false;
                            }
                        }
                        break;
                    case MirClass.Taoist:
                        if (!Item.Info.RequiredClass.HasFlag(RequiredClass.Taoist))
                        {
                            ...
                            {
                                GameScene.Scene.ChatDialog.ReceiveChat("This item requires a taoist-like attribute build.", ChatType.System);
                                return false;
                            }
                        }
                        break;
        ...
        private bool CanWearItem(UserObject actor, UserItem i)
        {
            ...
            // Attribute System - Check attribute requirements first (new system)
            if (i.Info.RequiredAttributes != null && i.Info.RequiredAttributes.Count > 0)
            {
                foreach (var attrReq in i.Info.RequiredAttributes)
                {
                    if (actor.AttributeValues == null || !actor.AttributeValues.ContainsKey(attrReq.Key))
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat($"You need {attrReq.Key} attribute level {attrReq.Value} to use this item.", ChatType.System);
            ...
            // Legacy class requirement check (backward compatibility)
            else if (i.Info.RequiredClass != RequiredClass.None)
            {
                MirClass requiredClassRole = Shared.Data.AttributeStatCalculator.DetermineClassRole(actor.AttributeValues);
